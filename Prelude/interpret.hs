...
main = do
    Text.putStrLn "[+] Reading"
    txt <- Text.readFile "test.annah"
    Text.putStrLn "[+] Parsing"
    Right ae <- return (Annah.exprFromText txt)
    Text.putStrLn "[+] Desugaring"
    me <- return $! Annah.desugar ae
    Text.putStrLn "[+] Resolving imports"
    me' <- Morte.load me
    Text.putStrLn "[+] Type-checking"
    let Right mt = Morte.typeOf (Morte.normalize me')
    True <- return (mt == t)
    Text.putStrLn "[+] Compiling"
    Text.writeFile "tmp.hs" (toHaskell (rename (Morte.normalize me')))
    system "stack ghc tmp.hs"
